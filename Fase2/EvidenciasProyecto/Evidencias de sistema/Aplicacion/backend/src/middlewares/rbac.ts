...
export function authorize(resource: string, action: string) {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!role) {
        return sendError(res, 'Rol no encontrado', 403)
      }
      ...
      return sendError(res, error.message || 'Error al verificar permisos', 500)
    }
  }
}
...
export function requireRole(...roleNames: string[]) {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      return sendError(res, error.message || 'Error al verificar rol', 500)
    }
  }
}
